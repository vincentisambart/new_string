...
{
    if (self->data_in_utf16 && UTF16_ENC(self->enc)) {
	if (index < 0) {
	    index += self->len;
	    if (index < 0) {
		return false;
...
static bool str_is_valid_encoding(string_t *self)
{
    ...
    if (self->data_in_utf16 || BINARY_ENC(self->enc)) {
	return true;
    }
